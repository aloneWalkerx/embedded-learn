 ...
 * LED1：PF10，板上绿色 LED
 *
 * ?? 两个引脚的"是否引出"列均为 N —— 没有引到排针，只能用板载 LED
 * ?? 点亮方式：低电平点亮（LED 阳极经限流电阻接 VCC，阴极接 GPIO）
...
/* LED1 的 GPIO 时钟使能
 * ?? 和 LED0 展开后是同一个宏（都在 GPIOF），这句是冗余的
...
#define LED0_TOGGLE()                       do{ HAL_GPIO_TogglePin(LED0_GPIO_PIN_TYPE, LED0_GPIO_PIN);}while(0)
#define LED1_TOGGLE()                       do{ HAL_GPIO_TogglePin(LED1_GPIO_PIN_TYPE, LED1_GPIO_PIN);}while(0)
