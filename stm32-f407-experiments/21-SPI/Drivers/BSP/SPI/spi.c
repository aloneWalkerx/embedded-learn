...
void HAL_SPI_MspInit(SPI_HandleTypeDef *hspi)
{
    ...
    {
        /* --- 步骤 1：使能时钟 --- */
        SPI1_SPI_CLK_ENABLE();        // 使能 SPI1 时钟
        SPI1_SCK_GPIO_CLK_ENABLE();   // 使能 GPIOB 时钟（SCK 在 PB3）
        SPI1_MISO_GPIO_CLK_ENABLE();  // 使能 GPIOB 时钟（MISO 在 PB4）
        SPI1_MOSI_GPIO_CLK_ENABLE();  // 使能 GPIOB 时钟（MOSI 在 PB5）
        
        /* --- 步骤 2：配置 SCK（PB3）引脚 --- */
        gpio_init_struct.Pin = SPI1_SCK_GPIO_PIN;          // PB3
        gpio_init_struct.Mode = GPIO_MODE_AF_PP;           // ★ 复用推挽输出
        gpio_init_struct.Pull = GPIO_PULLUP;               // 上拉
        gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;     // 高速
        gpio_init_struct.Alternate = SPI1_SCK_GPIO_AF;     // ★ AF5 = SPI1
        HAL_GPIO_Init(SPI1_SCK_GPIO_PORT, &gpio_init_struct);
        
        /* --- 步骤 3：配置 MISO（PB4）引脚 --- */
        gpio_init_struct.Pin = SPI1_MISO_GPIO_PIN;         // PB4
        gpio_init_struct.Mode = GPIO_MODE_AF_PP;           // ★ 复用推挽输出
        gpio_init_struct.Pull = GPIO_PULLUP;               // 上拉
        gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;     // 高速
        gpio_init_struct.Alternate = SPI1_MISO_GPIO_AF;    // ★ AF5 = SPI1
        HAL_GPIO_Init(SPI1_MISO_GPIO_PORT, &gpio_init_struct);
        
        /* --- 步骤 4：配置 MOSI（PB5）引脚 --- */
        gpio_init_struct.Pin = SPI1_MOSI_GPIO_PIN;         // PB5
        gpio_init_struct.Mode = GPIO_MODE_AF_PP;           // ★ 复用推挽输出
        gpio_init_struct.Pull = GPIO_PULLUP;               // 上拉
        gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;     // 高速
        gpio_init_struct.Alternate = SPI1_MOSI_GPIO_AF;    // ★ AF5 = SPI1
...
void spi1_set_speed(uint32_t speed)
{
    ...
    g_spi1_handle.Instance->CR1 &= ~SPI_CR1_BR_Msk;   // 清除分频位
    g_spi1_handle.Instance->CR1 |= speed;             // 设置新的分频值
