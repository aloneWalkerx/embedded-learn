#include "./BSP/ADC/adc.h"      // 包含 ADC 模块的头文件（宏定义、函数声明等）
...
void adc_init(void)
{
    ...
    // ADC_EOC_SINGLE_CONV：每个通道转换完成后，EOC 标志就置位
    // 本实验只转换一个通道，两者效果相同
    g_adc_handle.Init.EOCSelection = ADC_EOC_SEQ_CONV;
    
    // ⑦ 连续转换模式：禁用（单次转换）
    // 启用后，ADC 会连续不断地进行转换（无需重复触发）
    // 禁用时，每次需要软件或硬件触发才能启动一次转换
    // 本实验用软件触发，按一次按键转换一次，所以禁用
...
void adc_channel_set(ADC_HandleTypeDef *adc_handle, uint32_t channel, uint32_t rank, uint32_t sampling_time)
{
    ...
    // - ADC_SAMPLETIME_480CYCLES：  最慢，精度最高，适合高阻抗信号源（如 NTC）
