...
static uint8_t fonts_update_fontx(uint16_t x, uint16_t y, uint8_t size, uint8_t *fpath, uint8_t fx, uint16_t color)
{
    ...
    /* 从内存池分配文件对象结构体内存 */
    fftemp = (FIL *)mymalloc(SRAM_IN, sizeof(FIL));  /* 分配内存 */
    if (fftemp == NULL) rval = 1;   // 内存分配失败

    /* 从内存池分配4KB数据缓冲区 */
    tempbuf = mymalloc(SRAM_IN, 4096);               /* 分配4096个字节空间 */
    ...
    /* 释放之前分配的内存 */
    myfree(SRAM_IN, fftemp);     /* 释放内存 */
    myfree(SRAM_IN, tempbuf);    /* 释放内存 */
...
uint8_t fonts_update_font(uint16_t x, uint16_t y, uint8_t size, uint8_t *src, uint16_t color)
{
    ...
    /* 从内存池申请内存 */
    pname = mymalloc(SRAM_IN, 100);                          /* 申请100字节内存（存放文件路径） */
    buf = mymalloc(SRAM_IN, 4096);                           /* 申请4K字节内存（用于擦除校验） */
    fftemp = (FIL *)mymalloc(SRAM_IN, sizeof(FIL));          /* 分配文件对象内存 */

    /* 检查内存是否全部申请成功 */
    if (buf == NULL || pname == NULL || fftemp == NULL)
    {
        /* 释放已申请的内存 */
        myfree(SRAM_IN, fftemp);
        myfree(SRAM_IN, pname);
        myfree(SRAM_IN, buf);
        return 5;           /* 内存申请失败 */
    }
    ...
    myfree(SRAM_IN, fftemp); /* 释放文件对象内存 */
    ...
    if (rval == 0)          /* 字库文件都存在 */
    {
        ...
        for (i = 0; i < FONTSECSIZE; i++)           /* 遍历字库区域的每个扇区 */
        {
            ...
            /* 读出整个扇区的内容 */
            norflash_read_data((uint8_t *)buf, ((FONTINFOADDR / 4096) + i) * 4096, 4096);    /* 读出整个扇区的内容 */
...
uint8_t fonts_init(void)
{
    ...
    while (t < 10)  /* 连续读取10次,都是错误,说明确实是有问题,得更新字库了 */
    {
        ...
        norflash_read_data((uint8_t *)&ftinfo, FONTINFOADDR, sizeof(ftinfo));    /* 读出ftinfo结构体数据 */
