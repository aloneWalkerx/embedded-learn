...
char *usmart_get_input_string(void)
{
    ...
    if (g_usart_rec_status & 0x8000)        /* 串口接收完成？ */
    {
        len = g_usart_rec_status & 0x3fff;  /* 得到此次接收到的数据长度 */
        g_usart_rec_buf_max[len] = '\0';     /* 在末尾加入结束符. */
        pbuf = (char*)g_usart_rec_buf_max;
        g_usart_rec_status = 0;             /* 开启下一次接收 */
    }
