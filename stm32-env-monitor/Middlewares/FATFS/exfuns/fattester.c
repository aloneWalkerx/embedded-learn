...
{
    fattester.file = (FIL *)mymalloc(SRAM_IN, sizeof(FIL));      /* 为file申请内存 */
    fattester.fatbuf = (uint8_t *)mymalloc(SRAM_IN, 512);        /* 为fattester.fatbuf申请内存 */
...
void mf_free(void)
{
    myfree(SRAM_IN, fattester.file);
    myfree(SRAM_IN, fattester.fatbuf);
}
