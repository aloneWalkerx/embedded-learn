                                      ...
                                      uint32_t totsize, uint32_t cpdsize, uint8_t fwmode)
{
    ...
    fsrc = (FIL *)mymalloc(SRAM_IN, sizeof(FIL));    /* 申请内存 */
    fdst = (FIL *)mymalloc(SRAM_IN, sizeof(FIL));
    fbuf = (uint8_t *)mymalloc(SRAM_IN, 8192);
...
#define MAX_PATHNAME_DEPTH  512 + 1     /* 最大目标文件路径+文件名深度 */
    ...
    fddir = (DIR *)mymalloc(SRAM_IN, sizeof(DIR));   /* 申请内存 */
    finfo = (FILINFO *)mymalloc(SRAM_IN, sizeof(FILINFO));

    if (fddir == NULL || finfo == NULL)res = 100;

    if (res == 0)
    {
        pathname = mymalloc(SRAM_IN, MAX_PATHNAME_DEPTH);
        ...
        if (res == 0)
        {
            ...
            myfree(SRAM_IN, pathname);
        }
    }

    myfree(SRAM_IN, fddir);
    myfree(SRAM_IN, finfo);
...
#define MAX_PATHNAME_DEPTH 512 + 1  /* 最大目标文件路径+文件名深度 */
    ...
    srcdir = (DIR *)mymalloc(SRAM_IN, sizeof(DIR));  /* 申请内存 */
    dstdir = (DIR *)mymalloc(SRAM_IN, sizeof(DIR));
    finfo = (FILINFO *)mymalloc(SRAM_IN, sizeof(FILINFO));
    ...
    if (res == 0)
    {
        dstpathname = mymalloc(SRAM_IN, MAX_PATHNAME_DEPTH);
        srcpathname = mymalloc(SRAM_IN, MAX_PATHNAME_DEPTH);
        ...
        if (res == 0)
        {
            ...
            myfree(SRAM_IN, dstpathname);
            myfree(SRAM_IN, srcpathname);
        }
    }

    myfree(SRAM_IN, srcdir);
    myfree(SRAM_IN, dstdir);
    myfree(SRAM_IN, finfo);
    return res;
}
