...
void norflash_init(void)
{
    ...
    //根据参数初始化引脚信息
    HAL_GPIO_Init(NORFLASH_CS_GPIO_PIN_TYPE, &gpio_init_struct);
    // ★ 默认 CS 为高（不选中 Flash）
    NORFLASH_CS(1);   
    
    //步骤 3：初始化 SPI1 并设置速率
    // 初始化 SPI1（模式 3，8 位，MSB 先传）
    spi_init();
    // ★ 设置速率为 21MHz（84MHz/4）
    ...
    // ★ 只有 W25Q256 需要 4 字节地址模式
    if (g_norflash_type == W25Q256)
    {
        // 读取状态寄存器 3
        temp = norflash_read_sr(3);
        // ★ bit0 = ADS（地址模式选择），0=3字节，1=4字节
        if ((temp & 0x01) == 0)
        {
            // ★ 写使能（修改状态寄存器前必须使能）
            norflash_write_enable();
            // ★ bit1 = ADP（上电地址模式），1=4字节地址模式
            temp |= (1 << 1);
            // 写入状态寄存器 3
            ...
            NORFLASH_CS(0);
            // 发送 4 字节地址使能命令
            spi_read_write_byte(NORFLASH_Enable4ByteAddr);
            // 释放 Flash
            NORFLASH_CS(1);
        }
    }
...
void norflash_write_enable(void)
{
    ...
    NORFLASH_CS(0);
    //★ 发送写使能命令 0x06
    spi_read_write_byte(NORFLASH_WriteEnable);
    //失能NOR Flash片选
    NORFLASH_CS(1);
}
...
static void norflash_send_address(uint32_t address)
{
    ...
    {
        //发送 bit31~bit24 地址
        spi_read_write_byte((uint8_t)(address >> 24));
    }
    // ★ 所有型号都发送低 24 位地址
    //发送 bit23~bit16 地址
    spi_read_write_byte((uint8_t)(address >> 16));
    //发送 bit15~bit8 地址
    spi_read_write_byte((uint8_t)(address >> 8));
    //发送 bit7~bit0 地址
    spi_read_write_byte((uint8_t)address);
}
...
uint8_t norflash_read_sr(uint8_t regno)
{
    ...
    switch (regno)
    {
        ...
        {
            //0x05，读状态寄存器1
            command = NORFLASH_ReadStatusReg1;
            break;
        }
        case 2:
        {
            //0x35，读状态寄存器2
            command = NORFLASH_ReadStatusReg2;
            break;
        }
        case 3:
        {
            //0x15，读状态寄存器3
            command = NORFLASH_ReadStatusReg3;
            break;
        }
        default:
        {
...
void norflash_write_sr(uint8_t regno, uint8_t sr)
{
    ...
    switch (regno)
    {
        ...
        {
            //0x01，写状态寄存器1
            command = NORFLASH_WriteStatusReg1;
            break;
        }
        case 2:
        {
            //0x31，写状态寄存器2
            command = NORFLASH_WriteStatusReg2;
            break;
        }
        case 3:
        {
            //0x11，写状态寄存器3
            command = NORFLASH_WriteStatusReg3;
            break;
        }
        default:
        {
...
uint16_t norflash_read_id(void)
{
    ...
    NORFLASH_CS(0);
    //★ 发送读ID命令 0x90
    spi_read_write_byte(NORFLASH_ManufactDeviceID);
    //★ 发送 3 个哑字节（地址）
    spi_read_write_byte(0);
    spi_read_write_byte(0);
    spi_read_write_byte(0);
    //★ 读取高8位字节（制造商 ID）
    deviceid = spi_read_write_byte(0xFF) << 8;
    //★ 读取低8位字节（容量 ID）
    deviceid |= spi_read_write_byte(0xFF);
    //释放 Flash
    NORFLASH_CS(1);
 ...
 * @retval  无
 */
void norflash_read_data(uint8_t *pbuf, uint32_t addr, uint16_t datalen)
{
    uint16_t i;
    //选中 Flash
    NORFLASH_CS(0);
    //★ 发送读取命令 0x03
    spi_read_write_byte(NORFLASH_ReadData);
    //★ 发送 24 位（或 32 位）地址
...
static void norflash_write_page(uint8_t *pbuf, uint32_t addr, uint16_t datalen)
{
    ...
    NORFLASH_CS(0);
    //★ 发送页编程命令 0x02 
    spi_read_write_byte(NORFLASH_PageProgram);
    //★ 发送 24 位（或 32 位）地址
    norflash_send_address(addr);
    //写入 datalen 个字节
    for (i = 0; i < datalen; i++)
...
{
    // 当前扇区索引（0 = 第 1 个 4KB 扇区）
    uint32_t secpos;
    // 写入起始地址在扇区内的偏移量（0~4095）
    uint16_t secoff;
    // 当前扇区剩余的字节数
    uint16_t secremain;
    uint16_t i;
    ...
    while (1)
    {
        //步骤 1：读取整个扇区到缓存
        //读出当前整个扇区的内容
        norflash_read_data(norflash_buf, secpos * 4096, 4096);
...
{ 
    ...
    NORFLASH_CS(0);
    //★ 发送全片擦除命令 0xC7
    spi_read_write_byte(NORFLASH_ChipErase);
    //释放 Flash
    NORFLASH_CS(1);
    //★ 等待擦除完成（耗时很长！）
    norflash_wait_busy();
...
void norflash_erase_sector(uint32_t saddr)
{
    ...
    NORFLASH_CS(0);
    //★ 发送扇区擦除命令 0x20
    spi_read_write_byte(NORFLASH_SectorErase);
    //★ 发送 24 位（或 32 位）地址
